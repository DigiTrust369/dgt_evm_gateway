...
import "./contracts-upgradeable/utils/cryptography/MerkleProofUpgradeable.sol";
import "./contracts-upgradeable/proxy/utils/Initializable.sol";

import "./ERC1155NonTransferableBurnableUpgradeable.sol";

contract DigitrustsV7 is
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ERC1155NonTransferableBurnableUpgradeable
{
    ...
    string public constant CONTRACT_NAME = "Digitrust";
    ...
    /// @notice The EIP-712 typehash for the Digitrust input struct used by the contract
    bytes32 public constant DIGITRUST_TYPE_HASH =
        keccak256(
            "DIGITRUSTS(string headline,string description,uint256 startDateTimestamp,uint256 endDateTimestamp,string[] links,string communityUniqId,bool isSignatureRequired,bool isAllowlistRequired,int256 totalClaimCount,uint256 expirationTimestamp)"
        );
    ...
    /// @notice The EIP-712 typehash for adding new allowlisted addresses to an existing Digitrust token
    ...
    ////////////////////////////////// STRUCTS //////////////////////////////////
    /// @dev Struct used to contain the Digitrusts metadata input
    ...
    struct DigitrustsInputContainer {
        string headline;
        string description;
        ...
        DigitrustsContributorsInputContainer contributorMerkleRoots;
        DigitrustsClaimabilityAttributesInputContainer claimabilityAttributes;
    }

    /// @dev Struct used to contain the full Digitrusts metadata at the time of mint
    ///      Order of these variables should not be changed
    struct DigitrustsContainer {
        string headline;
        string description;
        uint256 startDateTimestamp;
        uint256 endDateTimestamp;
        ...
        string communityUniqId;
        DigitrustsClaimabilityAttributesContainer claimabilityAttributes;
        string customAttributes; // stringified JSON value that stores any other custom attributes
    }

    struct DigitrustsClaimabilityAttributesInputContainer {
    ...
    struct DigitrustsClaimabilityAttributesContainer {
        bool isSignatureRequired;
        bool isAllowlistRequired;
        int256 totalClaimCount; // -1 indicates infinite
    ...
    /// @dev Struct used to contain string and address Digitrusts contributors
    struct DigitrustsContributorsInputContainer {
    ...
    struct DigitrustsContributorsContainer {
    ...
    /// @dev Struct used by community admins to airdrop Digitrusts
    ...
    ///      which Digitrusts tokens without having to store the mapping on-chain.
    event RegisteredDigitrusts(address creator, uint256 tokenId);
    ...
    mapping(uint256 => DigitrustsContainer) public tokenIdToDigitrustsContainer;
    ...
    /// @dev Mapping from tokens to string and address Digitrusts contributors
    mapping(uint256 => DigitrustsContributorsContainer)
    ...
    function initialize(uint256 _latestUnusedTokenId) public initializer {
        __ERC1155_init("https://api.mintdigitrusts.xyz/metadata/{id}");
        __Ownable_init();
        __Pausable_init();
    ...
    /// @param newuri               The Digitrusts creator's address
    ...
    /// @notice Register new Digitrusts token type for contributors to claim AND airdrop that token to an initial address
    /// @dev Note that because we are using signed messages, if the Digitrusts input data is not the same as what it was at the time of user signing, the
    ...
    ///      the content of a Digitrusts.
    /// @param creator              The Digitrusts creator's address
    /// @param receiver             The Digitrusts receiver's address for airdrop
    /// @param metadata             Metadata of the Digitrusts token
    ...
    function registerBySigAndAirdrop(
        address creator,
        address receiver,
        DigitrustsInputContainer memory metadata,
        uint8 v,
        bytes32 r,
        bytes32 s
    ...
    /// @notice Register new Digitrust token type for contributors to claim.
    ...
    ///      Note that because we are using signed messages, if the Digitrust input data is not the same as what it was at the time of user signing, the
    ///      function call with fail. This ensures that whatever the user signs is what will get minted, and that we as the admins cannot tamper with
    ///      the content of a Digitrust.
    /// @param creator              The Digitrust creator's address
    /// @param metadata             Metadata of the Digitrust token
    /// @param v                    Part of the creator's signature (v)
    /// @param r                    Part of the creator's signature (r)
    /// @param s                    Part of the creator's signature (s)
    ...
    function _register(address creator, DigitrustsInputContainer memory metadata)
        internal
    {
        // Note that we currently don't have an easy way to de-duplicate Digitrusts tokens.
        // Because we are the only ones that can mint Digitrusts for now (since we're covering the cost),
        // we will gate duplicated tokens in the caller side.
        // However, once we open this up to the public (if the public wants to pay for their own Digitrusts at some point),
        // we may need to come up with some validation routine here to prevent the "same" Digitrusts from being minted.

        // Translate the Digitrusts input container to the actual container
        require(
        ...
        tokenIdToDigitrustsContainer[latestUnusedTokenId] = kc;

        emit RegisteredDigitrusts(creator, latestUnusedTokenId);
    ...
    /// @notice Only for community admins - Mints a Digitrusts to any consenting address
    ...
    function communityAdminAirdrop(
        ...
        DigitrustsContributorsInputContainer memory contributorMerkleRoots,
    ...
    /// @notice Only for community admins - Mints a Digitrusts to ANY address
    ...
    ///         Digitrusts to ANY address. This is intended for special cases where the regular
    ...
    /// @param id                                Token ID
    /// @param adminInput                        Container with the admin's consent info
    /// @param receivingAddress                  Address to mint the Digitrusts to
    /// @param updateContributorMerkleRoots      Flag to determine whether we should update the contributor merkle roots
    /// @param contributorMerkleRoots            New contributor merkle roots
    /// @param merkleProof                       Merkle proof for the particular claiming address
    ...
    function burn(
        ...
        require(
            tokenIdToDigitrustsContainer[id].creator != address(0),
    ...
    function claim(
        ...
        if (
            tokenIdToDigitrustsContainer[id]
                .claimabilityAttributes
                .isAllowlistRequired
        ) {
            require(
    ...
    function _claim(uint256 id, address dst) internal {
        ...
        bool hasFiniteCount = tokenIdToDigitrustsContainer[id]
            .claimabilityAttributes
            .totalClaimCount > 0;
        if (hasFiniteCount) {
            tokenIdToDigitrustsContainer[id]
    ...
    function _validateCommunityAdminAirdropAdminSig(
        ...
        string memory communityUniqId = tokenIdToDigitrustsContainer[id]
            .communityUniqId;
    ...
    function _tokenClaimChecks(uint256 id) internal view {
        ...
        bool hasExpirationSet = tokenIdToDigitrustsContainer[id]
            .claimabilityAttributes
        ...
        bool isAllowlistRequired = tokenIdToDigitrustsContainer[id]
            .claimabilityAttributes
            .isAllowlistRequired;
        bool hasClaimCountLimit = tokenIdToDigitrustsContainer[id]
            .claimabilityAttributes
            .totalClaimCount >= 0;
        uint256 remainingCount = tokenIdToDigitrustsContainer[id]
            .claimabilityAttributes
            .remainingClaimCount;
        require(
            isAllowlistRequired ||
    ...
    function modifyDigitrustsClaimAttributes(
        uint256 id,
        int256 totalClaimCount, // -1 indicates infinite
        uint256 expirationTimestamp, // 0 indicates no expiration
        bool isSignatureRequired,
    ...
    /// @notice Adds allowlisted addresses to an existing Digitrusts token. Note that this function is actually
    ...
    ///         to enable API integrations where partners want to add contributors to an existing Digitrusts token
    ///         programmatically.
    /// @param id                            ID of the Token
    ...
    function addAllowlistedAddressesBySig(
        uint256 id,
        DigitrustsContributorsInputContainer memory allowlistedContributorRoots,
        uint8 v,
        bytes32 r,
        bytes32 s
    ) public onlyOwner whenNotPaused {
        ...
        require(
            tokenIdToDigitrustsContainer[id].creator == signatory,
    ...
    function _addAllowlistedContributorRoots(
        uint256 id,
        DigitrustsContributorsInputContainer memory newAllowlistedContributorRoots
    ) internal {
    ...
    ///         existing Digitrusts tokens' contributor data.
    /// @param id                            ID of the Token
    /// @param allowlistedContributorRoots   Merkle roots of allowlisted contributors
    ...
    function getAllowlistedContributors(uint256 tokenId)
        public
        view
        returns (DigitrustsContributorsContainer memory)
    {
        return tokenIdToContributors[tokenId];
    }

    /// @notice Returns the Digitrusts metadata for a given token ID
    ...
    /// @param tokenId     ID of the token
    function getDigitrustsMetadata(uint256 tokenId)
        public
        view
        returns (DigitrustsContainer memory)
    {
        return tokenIdToDigitrustsContainer[tokenId];
    }
